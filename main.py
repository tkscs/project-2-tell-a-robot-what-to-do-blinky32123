...
def wall_check( left, right ):
    if left <25 and right <25:
        print ( "too close, go backward and stop")
        robot.motors(left=STOP, right=STOP, seconds=0.1)
        robot.motors(left=BACKWARD, right=BACKWARD, seconds=1.5)
        robot.motors(left=BACKWARD, right=FORWARD, seconds=1.4) #rotate 180
        return "stop" 
    elif left <25:
        print ( "turn right")
        robot.motors(left=STOP, right=STOP, seconds=0.1)
...
#tester
# for i in range(100):
#      robot.motors(right=FORWARD, left=FORWARD, seconds=1)
#      wall_check()
...
def star(): #72 degrees for star #wip
    for i in range(5):
        both_forward()
        robot.motors(left=BACKWARD, right=FORWARD, seconds=0.95)
...
def placeholder():
    question=input("what movement would you like the robot to do./n")
    if question == "swerve":
        swerve()
        print("swerving")
...
while robot_state!="off":
    robot_state = input("What would you like the robot to do? (check wall,idle,follow wall,off,8)")
    if robot_state=="check wall":
        ...
        idle()
    elif robot_state=="follow wall": #follow wall boundaries #wip
        print("The state is follow wall")
        follow_wall()
    elif robot_state=="off":
        print("robo terminated")
        break
    elif robot_state=="8":
